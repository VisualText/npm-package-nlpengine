 ...
 * Skips run/ when kbOnly; skips kb/ when analyzerOnly. Only .cpp and
 * .h files are copied.
 */
function stagePayload(analyzerDir, stageDir, kbOnly, analyzerOnly = false) {
  const subdirs = kbOnly ? ['kb'] : (analyzerOnly ? ['run'] : ['run', 'kb']);
  let anySource = false;
 ...
 * kb.<ext> (plus the "u" unicode variants) for a full build, kb.<ext>
 * alone for kbOnly, or run.<ext>/runu.<ext> for analyzerOnly. Returns
 * the bin/ directory path.
 */
async function stageArtifact(artifactUrl, analyzerDir, kbOnly, analyzerOnly = false) {
...
async function cloudCompile(engine, analyzerName, opts = {}) {
  ...
  if (kbOnly && analyzerOnly) {
    throw new CloudCompileError('kbOnly and analyzerOnly are mutually exclusive');
  }

  let analyzerDir;
  if (!skipLocalCompile) {
    analyzerDir = engine.compile(analyzerName, develop, kbOnly, analyzerOnly);
  } else {
    const base =
  ...
  let artifactUrl;
  try {
    stagePayload(analyzerDir, stageDir, kbOnly, analyzerOnly);
    ...
    console.error(
      `Uploading ${analyzerName} to ${dispatcherUrl} ` +
        `(engine=${engineVer} platform=${platformKey} kbOnly=${kbOnly} analyzerOnly=${analyzerOnly})`,
    );
  ...
  const binDir = await stageArtifact(artifactUrl, analyzerDir, kbOnly, analyzerOnly);
  // eslint-disable-next-line no-console
